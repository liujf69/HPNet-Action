...
class DecordInit:
    ...
    def __call__(self, results):
        ...
        if results['tar'] is False:
            ...
            file_obj = io.BytesIO(self.file_client.get(results['filename']+'.mp4'))
        else:
...
class Getdata:
    ...
    def __call__(self, results):
        ...
        num_files = len(os.listdir(results['filename']))
        results['total_frames'] = num_files
        return results

@PIPELINES.register_module()
class LoadRGB:
    def __init__(self, num_clips=1, train=True, test_mode=False, **kwargs):
    ...
    def __call__(self, results):
        ...
        ff = results['total_frames']
        video = results['filename']
        last_idx = ff - 1  # 默认最后一帧为样本的最后一帧

        if ff < self.num_clips:
        ...
        imgs = []
        if  self.train:
            for jj in range(num_clips):
                iii = int(np.random.randint(int(ff * jj / num_clips), int(ff * (jj + 1) / num_clips)))
                frame_idx = iii
                frame_path = os.path.join(video, str(iii) + '.jpg')
            ...
            for jj in range(num_clips):
                iii = int((int(ff * jj / num_clips) + int(ff * (jj + 1) / num_clips)) / 2)
                frame_idx = iii
                frame_path = os.path.join(video, str(iii) + '.jpg')
                frame = Image.open(frame_path)
                frame = np.array(frame)
        ...
        if ff < self.num_clips:
            frame_idx = last_idx
            frame_path = os.path.join(video, str(frame_idx) + '.jpg')
            frame = Image.open(frame_path)
            frame = np.array(frame)
            imgs.append(frame)
