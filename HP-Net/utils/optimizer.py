...
def set_weight_decay(model, skip_list=(), skip_keywords=(), weight_decay=0.001, lr=2e-6, have=(), not_have=()):
    has_decay = []
    no_decay = []
    not_grad_name = [] # new add
    have_name = [] # new add
    not_have_name = [] # new add
    no_decay_name = [] # new add
    has_decay_name = [] # new add
    for name, param in model.named_parameters():
        if not param.requires_grad:
            not_grad_name.append(name)
            continue  # frozen weights
        if len(have) > 0 and not check_keywords_in_name(name, have):
            have_name.append(name)
            continue
        if len(not_have) > 0 and check_keywords_in_name(name, not_have):
            not_have_name.append(name)
            continue
...
def fix_text(model):
    for name, param in model.named_parameters():
        if "visual." in name or "mit" in name or "prompts" in name:
            continue
        if "GCN" in name or "ske_film" in name or "ske_fc" in name or "video_film" in name: # new add
            continue
        else:
...
def build_optimizer(config, model):
    ...
    clip_parameters = set_weight_decay(model, skip, skip_keywords, 
        ...
        have=(), not_have=("prompts", "mit", "message_", "GCN", "ske_film", "ske_fc", "video_film") # new add GCN and ske_film
    )
    ...
    # new add
    gcn_parameters = set_weight_decay(model, skip, skip_keywords,
        weight_decay=config.TRAIN.WEIGHT_DECAY, lr=config.TRAIN.LR*10000, # 10000
        have=("GCN",), not_have=()
    )
    film_parameters = set_weight_decay(model, skip, skip_keywords,
        weight_decay=config.TRAIN.WEIGHT_DECAY, lr=config.TRAIN.LR*10000, # 10000
        have=("ske_film", "ske_fc", "video_film"), not_have=()
    )
    # optimizer = optim.AdamW(clip_parameters + mit_parameters + prompts_parameters + msg_parameters,
    #                     betas=(0.9, 0.98), eps=1e-3,) # old
    optimizer = optim.AdamW(clip_parameters + mit_parameters + prompts_parameters + msg_parameters + gcn_parameters + film_parameters,
                        betas=(0.9, 0.98), eps=1e-3,) # new 
