...
class MultiframeIntegrationTransformer(nn.Module):
    ...
    def forward(self, x): # video features, x: [B, T, 512]
        ori_x = x # [B, T, 512]
        x = x + self.positional_embedding # [B, T, 512]
        x = x.permute(1, 0, 2)
        x = self.resblocks(x) # nn.MultiheadAttention() + MLP
        x = x.permute(1, 0, 2)  
        x = x.type(ori_x.dtype) + ori_x # [B, T, 512]
        
        return x.mean(dim=1, keepdim=False) # [B, 512]
