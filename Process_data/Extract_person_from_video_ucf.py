...
def get_parser():
    ...
    parser.add_argument(
        '--output_path', 
        type = str,
        default = '/data/liujinfu/TPAMI-24/ucf/output')
    parser.add_argument(
        '--device', 
        type = int,
        default = 4)
...
def Extract_Person_frame(root_video_path, output_path, model_path, data_yaml, device):
    ...
    for _, action_name in enumerate(action_names):
        ...
        for _, video in enumerate(action_videos): 
                ...
                save_path = os.path.join(os.path.join(output_path, action_name), video).split('.')[0]
                ...
                while ret:
                    ...
                    if (not ret):
                        break
                    img, num = model.extract_person(rgb_img)
...
if __name__ == "__main__":
    # nohup python Extract_person_from_video_ucf.py --device 4 > /data/liujinfu/TPAMI-24/ucf/process_dataset.txt 2>&1 &
    parser = get_parser()
    ...
    device = args.device
    Extract_Person_frame(root_video_path, output_path, model_path, data_yaml, device)
    print("All done!")