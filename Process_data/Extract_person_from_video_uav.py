...
def get_parser():
    ...
    parser.add_argument(
        '--output_path', 
        type = str,
        default = '/data/liujinfu/dataset/UAV/person_224')
    parser.add_argument(
        '--device', 
        type = int,
        default = 0)
...
def Extract_Person_frame(samples, video_path, output_path, model_path, data_yaml, device):
    ...
    for _, name in enumerate(samples):
        ...
        video_file_path = os.path.join(video_path, name.split(".txt")[0] + ".avi")
        save_path = os.path.join(output_path, name.split(".txt")[0])
...
if __name__ == "__main__": 
    # nohup python Extract_person_from_video_uav.py --sample_name_path /data/liujinfu/dataset/UAV/Process_Pose/new_CS_train_V1.txt > /data/liujinfu/TPAMI-24/Process_data/outlog/uav_train_v1.log 2>&1 &
    # nohup python Extract_person_from_video_uav.py --sample_name_path /data/liujinfu/dataset/UAV/Process_Pose/new_CS_test_V1.txt > /data/liujinfu/TPAMI-24/Process_data/outlog/uav_test_v1.log 2>&1 &
