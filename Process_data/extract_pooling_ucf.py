...
def parse_args():
    ...
    parser.add_argument('--videos_path',
                    ...
                    default = "/data/liujinfu/TPAMI-24/ucf/UCF-101")
    parser.add_argument('--save_path',
                    help = "the save path of pooling features",
                    default = "./output/pooling_feature_ucf")
...
def main(args):
    ...
    root_video_path = args.videos_path
    output_path = args.save_path
    
    action_names = os.listdir(root_video_path) # 动作类别

    for _, action_name in enumerate(action_names):
        action_videos_path = os.path.join(root_video_path, action_name)
        action_videos = os.listdir(action_videos_path)
        for _, video in enumerate(action_videos):
            video_file_path = os.path.join(action_videos_path, video)
            print("Process ", video_file_path)
            
            save_path = os.path.join(os.path.join(output_path, action_name), video).split('.avi')[0]
            if not os.path.exists(os.path.join(output_path, action_name)):
                os.makedirs(os.path.join(output_path, action_name))
...
if __name__ == "__main__":
    # nohup python extract_pooling_ucf.py --videos_path /data/liujinfu/TPAMI-24/ucf/UCF-101 --save_path ./output/pooling_feature_ucf --device 0 > /data/liujinfu/TPAMI-24/ucf/process_ucf_pooling.log 2>&1 &
