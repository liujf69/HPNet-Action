...
def parse_args():
    ...
    parser.add_argument('--save_path',
                    ...
                    default = "/data/liujinfu/dataset/UAV/pooling")
    parser.add_argument('--sample_txt',
                        type = str,
                        default = '/data/liujinfu/dataset/UAV/Process_Pose/new_CS_train_V1.txt')
...
def main(args):
    ...
    for _, name in enumerate(samples_name): # for each sample
        ...
        rgb_video_path = os.path.join(videos_path, name.split(".txt")[0] + ".avi")
        ...
        np.save(save_path + "/" + name.split(".txt")[0] + ".npy", save_features) 
        
        # pose_save_path = "/data/liujinfu/dataset/UAV/HPE_Pose" + "/" + name.split(".txt")[0] + ".npy"  
        # np.save(pose_save_path, origin_Pose_data)
    print("All done!")
    
if __name__ == "__main__":
    # nohup python extract_pooling_uav.py --sample_txt /data/liujinfu/dataset/UAV/Process_Pose/new_CS_train_V1.txt > /data/liujinfu/TPAMI-24/Process_data/outlog/Pooling_UAV_train_V1.log 2>&1 &
    # nohup python extract_pooling_uav.py --sample_txt /data/liujinfu/dataset/UAV/Process_Pose/new_CS_test_V1.txt > /data/liujinfu/TPAMI-24/Process_data/outlog/Pooling_UAV_test_V1.log 2>&1 &
