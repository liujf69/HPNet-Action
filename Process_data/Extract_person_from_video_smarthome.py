...
def get_parser():
    ...
    parser.add_argument(
        '--output_path', 
        type = str,
        default = '/data/liujinfu/dataset/Toyota-Smarthome/person_224')
    parser.add_argument(
        '--device', 
        type = int,
        default = 2)
...
def Extract_Person_frame(samples, video_path, output_path, model_path, data_yaml, device):
    ...
    for _, name in enumerate(samples):
        ...
        video_file_path = os.path.join(video_path, name)
        save_path = os.path.join(output_path, name.split('.mp4')[0])
...
if __name__ == "__main__":
    # nohup python Extract_person_from_video_smarthome.py --sample_name_path /data/liujinfu/dataset/Toyota-Smarthome/train_CS.txt --device 2 > ./outlog/Smarthome_train_CS.log 2>&1 &
    # nohup python Extract_person_from_video_smarthome.py --sample_name_path /data/liujinfu/dataset/Toyota-Smarthome/validation_CS.txt --device 2 > ./outlog/Smarthome_validation_CS.log 2>&1 &
    # nohup python Extract_person_from_video_smarthome.py --sample_name_path /data/liujinfu/dataset/Toyota-Smarthome/test_CS.txt --device 2 > ./outlog/Smarthome_test_CS.log 2>&1 &
